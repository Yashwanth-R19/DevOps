...
Every push to `main` trains a random forest regressor on the Boston Housing
dataset, evaluates it, and — only if it passes an R² threshold — deploys it
to Hugging Face Hub automatically.
...
push/PR to GitHub
  ...
  └──────┬──────────────┘  ← fails here if R² < params.yaml threshold
         ▼
  ┌─────────────────────┐
...
- Name: `HF_REPO_ID`
- Value: `your-hf-username/your-model-name` (e.g. `mupalaniappan/boston-housing-rf`)
...
Edit `evaluate.min_r2` in `params.yaml`. Anything scoring below that R²
